...
node * node::insert(int l, int r, int w)
{
	if(l <= w && w < r)
	{
		...
		cout<< " left " << l << " right " << r << " weight " << w << " count " << this->count +1 << endl;
		if(l+1 == r)
		{
		...
		int m = (l+r)>>1;
        cout << "-->  " << this << " " <<this->left << " " << this->right << endl;
         {  node*tp = new node(this->count+1, this->left->insert(l, m, w), this->right->insert(m, r, w));
            cout<<tp<<endl;
            return tp ; }
	}
...
{
    cout<<"  cur node "<< cur << " prev node " << prev <<" weight " << M[v[cur]] << endl;
	pa[cur][0] = prev;
	...
	//cout << null << " " << null->left << " " << null->right << endl;
...
int main()
{
	...
	{
	    cout<< " node wieght " << it->first  <<" maxi " << maxi << endl  ;
		M[it->first] = maxi;
		RM[maxi] = it->first;
		maxi++;
	}
